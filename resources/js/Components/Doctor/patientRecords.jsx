...
function RecordDetailsDrawer({ record, onClose }) {
  return (
    <div
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-4">
          <div>
            ...
            <h2 id="record-details-title" className="mt-1 text-xl font-bold text-slate-900">
        ...
        <div className="flex-1 space-y-5 pt-5">
          <div className="flex flex-wrap items-center gap-2">
            <Badge tone={record.documentType || "record"}>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <RecordInfo label="Patient" value={record.patientName} />
            <RecordInfo label="Doctor" value={record.doctorName || record.doctor} />
            <RecordInfo label="Date" value={record.date || record.documentDate} />
            <RecordInfo label="Reference" value={record.referenceNo || record.invoiceNo} />
            <RecordInfo
              label="Amount"
              value={record.amountCents ? formatCurrency(record.amountCents, record.currency) : "Not available"}
            />
            <RecordInfo label="File" value={record.fileName || (record.fileUrl ? "Available online" : "No file attached")} />
          </div>
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            ...
            <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">
              {record.summary || record.notes || "No notes available."}
            </p>
          </div>
        </div>
...
function RecordInfo({ label, value }) {
  return (
    <div className="rounded-lg border border-slate-100 bg-slate-50 p-3">
      <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{label}</p>
      <p className="mt-1 text-sm font-medium text-slate-800">{value || "Not available"}</p>
    </div>
  );
}
...
async function downloadDocument(item) {
  ...
  if (item.fileUrl && item.fileUrl !== "#") {
    ...
    try {
      ...
      // A direct link still works when the file host does not allow CORS.
...
async function shareDocument(item) {
  ...
  const shareText = `${item.title || "Medical record"}${item.fileUrl && item.fileUrl !== "#" ? ` - ${item.fileUrl}` : ""}`;
  ...
  if (navigator.share) {
    try {
      await navigator.share({
        ...
        text: shareText,
        url: item.fileUrl && item.fileUrl !== "#" ? item.fileUrl : window.location.href,
      });
      return;
    } catch (error) {
  ...
  try {
    ...
    window.prompt("Copy this record information:", shareText);
  }
}
...
function openPrintableWindow(item, shouldPrint = false) {
  ...
  if (!popup) {
    window.alert("Please allow popups to view or print this record.");
    return;
  }
