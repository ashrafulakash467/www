      ...
      <div className="flex flex-col justify-between gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">
            Prescriptions, notes, invoices, and uploaded PDFs are synced from
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0 space-y-2">
                    ...
                    <p className="text-xs text-slate-500">
                      {getRecordMeta(item, recordCategory)}
                    </p>
                    {item.summary ? (
...
function getRecordMeta(item, recordCategory) {
  ...
  if (item.patientName && recordCategory !== "invoices") {
...
function downloadDocument(item) {
  ...
  const blob = new Blob([buildDownloadText(item)], { type: "text/plain;charset=utf-8" });
...
<body>
  <div class="sheet">
    <div class="eyebrow">${escapeHtml(docType)}</div>
    <h1>${escapeHtml(item.title || "Document")}</h1>
    <p class="meta">${escapeHtml(getRecordMeta(item, item.documentType || ""))}</p>
    <div class="grid">
      <div class="card"><div class="label">Type</div><div class="value">${escapeHtml(docType)}</div></div>
      <div class="card"><div class="label">Reference</div><div class="value">${escapeHtml(item.referenceNo || item.invoiceNo || "N/A")}</div></div>
      <div class="card"><div class="label">Patient</div><div class="value">${escapeHtml(item.patientName || "Patient")}</div></div>
      <div class="card"><div class="label">Doctor</div><div class="value">${escapeHtml(item.doctor || item.doctorName || "Doctor")}</div></div>
      <div class="card"><div class="label">Date</div><div class="value">${escapeHtml(item.date || "N/A")}</div></div>
      <div class="card"><div class="label">Amount</div><div class="value">${escapeHtml(amount || "N/A")}</div></div>
    </div>
    <div class="card">
...
function buildDownloadText(item) {
  return [
